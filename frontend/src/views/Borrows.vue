...
const loadRecords = async (page = 1, size = 10, studentId = null) => {
  try {
    const res = await borrowApi.getRecordsWithPagination(page, size, studentId)
...
const handleReset = () => {
  ...
  loadRecords(1, pageSize.value, null)
}

const handlePageChange = (page) => {
  loadRecords(page, pageSize.value, selectedStudent.value)
}

const handleSizeChange = (size) => {
  pageSize.value = size
  loadRecords(1, size, selectedStudent.value)
}
...
const handleStudentChange = async () => {
  loadRecords(1, pageSize.value, selectedStudent.value)
}
